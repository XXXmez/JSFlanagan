...
// 4.10. Логические выражения
    ...
    addContent('Логические выражения', 'Пример логических выражений: true && !false || 5');
...
// 4.11. Выражения присваивания
    //  Операция = в JavaScript применяется для присваивания значения переменной
    // или свойству, например:
        let zf = 1;
    // addContent('Логические выражения', 'Пример логических выражений: true && !false || 5');

    // Операция = ожидает
        // что ее операнд с левой стороны будет левым значением: переменной или свойством объекта (или элементом массива)

        // С правой стороны она ожидает, что операнд будет произвольным значением любого типа

    // Хотя выражения присваивания обычно довольно просты
    // иногда вы можете видеть, что значение выражения присваивания используется в более крупном выражении
        let agh;
        console.log((agh = 0) === 0);       // => true
    // Поступая так, убедитесь в том, что четко понимаете разницу между операциями = и ===!

    // операция = обладает очень низким приоритетом и в случае, когда ее значение применяется 
    // как часть более крупного выражения, обычно нужны круглые скобки

    // Операция присваивания имеет ассоциативность справа налево
        let fgh = grr = ggt = ffa = 7;
        console.log(fgh);       // => 7

    // 4.11.1. Присваивание с действием
        // в JavaScript поддерживается несколько других операций присваивания, которые 
        // обеспечивают сокращения за счет комбинирования присваивания с рядом других действий
            let tthh = 4;
            tthh += 9;
            // Эквивалентно
            tthh = tthh + 9;
        
        // Так же Похожие операции включают -=, *=, &=
...
// 4.13. Смешанные операции
    // 4.13.1. Условная операция ( ? : )
        // Условная операция — единственная операция с тремя операндами в JavaScript,
        // которая иногда называется тернарной операцией
            console.log(1==1 ? true : false);
        // Это аналог оператора if хорошее его сокращение

        let username;
        greeting = "hello " + (username ? username : "there 1");
    ...
    // 4.13.2. Операция выбора первого определенного операнда (??)
        // Операция выбора первого определенного операнда ?? вычисляется как ее первый определенный операнд
        // если левый операнд не является null или undefined, тогда она возвращает его значение.
    
        console.log("0 ?? 2:                    ", 0 ?? 2);
        console.log("false ?? true:             ", false ?? true);
        console.log("null ?? true:              ", null ?? true);
        console.log("null ?? undefined ?? 'G':  ", null ?? undefined ?? 'G');

        // Проблема с таким идиоматическим применением состоит в том, что ноль,
        // пустая строка и f a l s e — ложные значения, которые в определенных обстоятельствах
        // могут быть совершенно допустимыми

        let options = { timeout: 0, title:"", verbose: false, n: null };
        console.log("options.timeout ?? 1000:       ", options.timeout ?? 1000);            // => 0: как определено в объекте
        console.log("options.title ?? \"Untitled\":   ", options.title ?? "Untitled");      // => "" как определено в объекте
        console.log("options.verbose ?? true:       ", options.verbose ?? true);            // => false: как определено в объекте
        console.log("options.quiet ?? false:        ", options.quiet ?? false);             // => false: свойство не определено
        console.log("options.n ?? 10:               ", options.n ?? 10);                    // => 10: свойство равно null

        // У Операции ?? такой же приоритет как и у && и ||
        // Для указания последовательности действий нужно использовать ()
            // (а ?? b) || с   // Сначала ??, затем | |
            // а ?? (b || с)   // Сначала | |, затем ??
            // а ?? b || с     // SyntaxError: круглые скобки обязательны
            
    // 4.13.3. Операция typeof
        // typeof — унарная операция, помещаемая перед своим единственным операндом,
        // который может быть любого типа.
        console.log(typeof 1);      // => number
        console.log(typeof '1');    // => string
    
    // 4.13.4. Операция delete
        //  delete — унарная операция, которая пытается удалить свойство объекта
        // или элемент массива, указанный в качестве ее операнда
            let arrayD = [1,2,3];   // Создали массив
            console.log(arrayD);    // => [1, 2, 3]
            delete arrayD[1];       // удалили второй элемент
            console.log(arrayD);    // => [1, пусто, 3]
        // ВАЖНО удаленное свойство или элемент массива становится не undefined оно перестает существовать. в массиве оно создает пустое место

    //  4.13.5. Операция await
        // Операция aw a it появилась в ES2017 как способ сделать асинхронное программирование
        // в JavaScript более естественным
        // await ожидает в качестве своего единственного операнда объект Promise
        // и заставляет программу вести себя так, будто она ждет окончания асинхронного вычисления

    // 4.13.6. Операция v o id
        // void — унарная операция, располагающаяся перед своим операндом, который может быть любого типа
        // она вычисляет свой операнд, после чего отбрасывает значение и возвращает undefined
        // Для нее даже пример использования придумать нелегко
            let counter13 = 0;
            const increment13 = () => {void counter13++};
            console.log(increment13());     // => undefined
            console.log(counter13);         // => 1

    // 4.13.7. Операция "запятая"
        // Операция “запятая” (,) является бинарной операцией, чьи операнды могут
        // быть любого типа. Она вычисляет свой левый операнд, затем правый операнд и
        // далее возвращает значение правого операнда
            let lk = 0, lj = 1, lh = 2;
